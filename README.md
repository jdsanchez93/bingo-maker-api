...
```bash
bingo-maker-api$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
bingo-maker-api$ sam local start-api
bingo-maker-api$ curl http://localhost:3000/
```
...
```bash
bingo-maker-api$ sam logs -n NetCodeWebAPIServerless --stack-name bingo-maker-api --tail
```
...
```bash
bingo-maker-api$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name bingo-maker-api
```
